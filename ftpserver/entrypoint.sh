...
if [ -e /etc/ssl/private/pure-ftpd.pem ] && [[ "$PURE_FTPD_FLAGS" != *"--tls"* ]]; then
  echo "TLS Enabled"
...
if [ -e /etc/ssl/private/pure-ftpd-cert.pem ] && [ -e /etc/ssl/private/pure-ftpd-key.pem ] && [[ "$PURE_FTPD_FLAGS" == *"--tls"* ]]; then
...
if [ ! -e /etc/ssl/private/pure-ftpd.pem ] && [[ "$PURE_FTPD_FLAGS" == *"--tls"* ]] && [ ! -z "$TLS_CN" ] && [ ! -z "$TLS_ORG" ] && [ ! -z "$TLS_C" ]; then
...
$FTP_USER_PASS" >"$PWD_FILE"
  ...
  if [ ! -z "$FTP_USER_UID" ]; then
    if ! [[ $(ls -ldn $FTP_USER_HOME | awk '{print $3}') = $FTP_USER_UID ]]; then
      chown $FTP_USER_UID "$FTP_USER_HOME"
      echo " root user give $FTP_USER_HOME directory $FTP_USER_UID owner"
    fi
  else
    if ! [[ $(ls -ld $FTP_USER_HOME | awk '{print $3}') = 'ftpuser' ]]; then
